...
function renderCards() {
  ...
  articleGrid.innerHTML = filtered
    .map((item) => {
      ...
      const meta = [item.source, item.publishedLabel || item.publishedAt, item.duration].filter(Boolean).join(" · ");
...
function openReader(item) {
  ...
  readerMeta.textContent = [item.publishedLabel || item.publishedAt, item.contentType === "video_digest" ? "视频精读" : "文章索引", item.duration]
    .filter(Boolean)
    .join(" · ");
